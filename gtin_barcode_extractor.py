...
def process_image(
    image_path: str, gemini_key: str = None, gemini_model: str = "gemini-2.0-flash"
) -> tuple[str, str]:
...
def main():
    ...
    parser.add_argument(
        "--gemini-key", help="Google Gemini API Key for fallback processing", default=None
    )
    ...
    try:
        for file_path in tqdm(files_to_process, desc="Scanning images"):
            gtin, method = process_image(
                str(file_path), gemini_key=args.gemini_key, gemini_model=args.gemini_model
            )
            product_info = analyze_product_gemini(
                str(file_path), api_key=args.gemini_key, model=args.gemini_model
            )
